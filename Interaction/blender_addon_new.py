...
def get_controller_root():
    ...
    env_root = os.environ.get("FLS_CONTROLLER_ROOT")
    if env_root:
        candidates.append(env_root)

    home_dir = os.path.expanduser("~")
    candidates.extend([
        os.path.join(home_dir, "Documents", "FLS_Research", "fls-cf-offboard-controller"),
        os.path.join(home_dir, "Documents", "Holodeck", "fls-cf-offboard-controller"),
    ])

    try:
        addon_repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
        candidates.extend([
            os.path.join(addon_repo_root, "fls-cf-offboard-controller"),
            os.path.join(os.path.dirname(addon_repo_root), "fls-cf-offboard-controller"),
        ])
    except NameError:
        pass

    for candidate in candidates:
        if candidate and os.path.isfile(os.path.join(candidate, "orchestrator.py")):
            return candidate

    return candidates[0] if candidates else os.path.join(home_dir, "Documents", "FLS_Research", "fls-cf-offboard-controller")


def get_default_interaction_yaml_path(mission_name):
    return os.path.join(get_controller_root(), "Interaction", "mission", f"{mission_name}.yaml")


def get_default_orchestrator_path():
    return os.path.join(get_controller_root(), "orchestrator.py")


def get_controller_python():
    controller_root = get_controller_root()
    candidates = [
...
class DroneProperties(bpy.types.PropertyGroup):
    ...
    interaction_sd: FloatProperty(
        name="S_D (mm/s)",
        description="Interaction speed threshold exported as delta_v in m/s",
        default=100.0,
        min=0.0
    )
    ...
    interaction_duration: FloatProperty(
        name="Duration (s)",
        description="Interaction duration and per-drone delta_t",
        default=60.0,
        min=0.0
    )
...
def build_interaction_yaml_text(scene, drones_to_export):
    props = scene.drone_props
    mission_name = props.export_mission_name.strip() or scene.name.replace(' ', '_')
    duration = round(props.interaction_duration, 4)
    ...
    output_lines.extend([
        ...
        f"    duration: {duration}",
        "    v_scalar: [10, 10, 3]",
        f"    blender_port: {props.interaction_tcp_port}",
...
class EXPORT_OT_interaction_yaml(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        props = context.scene.drone_props
        self.filepath = get_default_interaction_yaml_path(props.export_mission_name.strip() or "interaction_mission")
    ...
    def execute(self, context):
        ...
        try:
            with open(self.filepath, 'w', encoding='utf-8') as f:
                f.write(build_interaction_yaml_text(scene, drones_to_export))
...
class EXPORT_OT_export_and_illuminate(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        target_yaml = "/Users/shuqinzhu/Documents/FLS_Research/fls-cf-offboard-controller/mission/blender_mission.yaml"
        target_script = "/Users/shuqinzhu/Documents/FLS_Research/fls-cf-offboard-controller/orchestrator.py"
...
class EXPORT_OT_export_and_interact(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        target_yaml = get_default_interaction_yaml_path(props.export_mission_name.strip() or "interaction_mission")
        target_script = get_default_orchestrator_path()
...
class DRONE_OT_start_static_illumination_edit(bpy.types.Operator):
    """Launch interaction mode and start a non-blocking TCP listener for static editing"""
    bl_idname = "drone.start_static_illumination_edit"
    bl_label = "Edit Static Illumination"
    ...
    def execute(self, context):
        ...
        result = bpy.ops.drone.export_and_interact('EXEC_DEFAULT')
        if 'FINISHED' not in result:
            close_static_interaction_session()
            props.interaction_editor_active = False
            return {'CANCELLED'}

        self.report({'INFO'}, f"Static editor listening on TCP port {props.interaction_tcp_port}. Waiting for LightBender connections.")
        return {'FINISHED'}


class DRONE_OT_record_interaction_positions(bpy.types.Operator):
    """Request current positions over TCP and update matching LightBenders"""
    bl_idname = "drone.record_interaction_positions"
    bl_label = "Record Position"
    ...
    def execute(self, context):
        ...
        if not props.interaction_editor_active:
            self.report({'ERROR'}, "Static interaction editor is not active.")
            return {'CANCELLED'}
        ...
        if not STATIC_INTERACTION_SESSION["sockets"]:
            show_popup(context, "Record Position", ["No LightBender connections are available yet."], icon='INFO')
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, "Requested positions. Waiting for responses.")
        return {'FINISHED'}


class DRONE_OT_disconnect_static_illumination_edit(bpy.types.Operator):
    """Close static interaction editor sockets"""
    bl_idname = "drone.disconnect_static_illumination_edit"
    bl_label = "Disconnect Static Editor"

    def execute(self, context):
        close_static_interaction_session()
        context.scene.drone_props.interaction_editor_active = False
        context.scene.drone_props.interaction_connected_count = 0
        context.scene.drone_props.interaction_recording_active = False
        self.report({'INFO'}, "Static interaction editor disconnected.")
        return {'FINISHED'}
...
class VIEW3D_PT_lb_export(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.operator("drone.export_yaml", text="Export YAML", icon='EXPORT')
        row.operator("drone.export_interaction_yaml", text="Export Interaction", icon='EXPORT')

        row = layout.row(align=True)
        row.operator("drone.export_and_illuminate", text="Illuminate", icon='PLAY')
        row.operator("drone.export_and_interact", text="Illuminate + Interact", icon='PLAY')


class VIEW3D_PT_lb_interaction(bpy.types.Panel):
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "LightBender"
    bl_label = "Interaction Config"
    ...
    def draw(self, context):
        ...
        layout.prop(props, "interaction_sd")
        layout.prop(props, "interaction_duration")
        layout.prop(props, "interaction_grace_time")

        layout.separator()
        layout.label(text="Static Illumination Editor:")
        layout.prop(props, "interaction_lightbender_ips")
        layout.prop(props, "interaction_tcp_port")

        if props.interaction_editor_active:
            if props.interaction_connected_count > 0:
                layout.label(text=f"{props.interaction_connected_count} LightBender connection(s).", icon='LINKED')
            else:
                layout.label(text="Listening for LightBender connections...", icon='TIME')

            if props.interaction_recording_active:
                layout.label(text="Waiting for position responses...", icon='INFO')

            row = layout.row(align=True)
            row.enabled = props.interaction_connected_count > 0 and not props.interaction_recording_active
            row.operator("drone.record_interaction_positions", text="Record Position", icon='TRACKER')
            row = layout.row(align=True)
            row.operator("drone.disconnect_static_illumination_edit", text="Disconnect", icon='CANCEL')
        else:
            layout.operator("drone.start_static_illumination_edit", text="Edit Static Illumination", icon='LIGHT')
...
classes = (
    ...
    DRONE_OT_start_static_illumination_edit,
    DRONE_OT_record_interaction_positions,
    DRONE_OT_disconnect_static_illumination_edit,
